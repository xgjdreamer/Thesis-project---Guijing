...
def get_vo_average_speaker_temporal_n(speaker, speaker_dic, length, exclude_stops):
    ...
    for i in range(0,slot_number):
        vo_dic = get_vo_average_speaker_in_window(speaker,speaker_dic, length*(i), length*(i+1), exclude_stops)
        vo_dic_list.append(vo_dic)

    vo_dic_end = get_vo_average_speaker_in_window(speaker, speaker_dic, slot_number*length, total_u, exclude_stops)
